...
void NativeLibrary::initialize() { hybris_set_hook_callback(hook_callback); }

void NativeLibrary::addGlobalHook(std::string const &symbol, void *replacement) {
	hooked_functions[symbol] = replacement;
}

void NativeLibrary::hookSymbol(std::string const &symbol, void *replacement) { hooks[symbol] = replacement; }
...
NativeLibrary::NativeLibrary(std::string const &path) {
	...
	try {
		auto JNI_OnLoad = (jint(*)(JavaVM * vm, void *reserved)) loadSymbol("JNI_OnLoad");
	...
	hooked_classes[name.filename()] = [this](std::string const &symbol) { return this->getHookedSymbol(symbol); };
	;
...
void *NativeLibrary::getHookedSymbol(const std::string &symbol) { return hooks[symbol]; }
...
NativeLibrary::~NativeLibrary() { hybris_dlclose(handle); }

LibraryLoadException::LibraryLoadException(const std::string &message) { text = message; }

const char *LibraryLoadException::what() const throw() { return text.c_str(); }
